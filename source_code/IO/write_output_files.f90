SUBROUTINE write_output_files(u,Temp,Chem,t,dt,istep)
  ...
  IF (MOD(istep,n_comp_diag)==0)             &       !Write out diagnostics
       &   CALL write_diagnostics_file(u,Temp,Chem,istep,t,dt)
  IF (MOD(istep,n_wrt_spec)==0)            &         !Write out horizontal spectra
       &   CALL  write_horizontal_spectra(u,Temp,Chem,istep,t)
  IF (MOD(istep,n_wrt_spec)==0)            &          !Write out vertical spectra
       &   CALL  write_vertical_spectra(u,Temp,Chem,istep,t)
  IF (MOD(istep,n_wrt_prof)==0)            &          !Write out vertical profiles
       &   CALL  write_z_profile(u,Temp,Chem,istep,t)
  ...
  IF (MOD(istep,n_wrt_dump)==0)              &       !Write restart file (netCDF)
       &   CALL pn_write_dump(u,Temp,Chem,t,dt,istep)
  IF (MOD(istep,n_wrt_netCDF)==0)            &       !Write simulation data file (netCDF)
       &   CALL pn_write_step_simdat_file(u,Temp,Chem,istep,t,dt)
