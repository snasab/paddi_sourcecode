...
SUBROUTINE collect_phys_single(local,global,sourcetask)
  ...
  IF (myid.EQ.sourcetask) THEN     
     ...
     ENDDO
  ELSE 
     ALLOCATE(work(1),recvcounts(1),rdispl(1)) !Some compiler complain if these are not allocated at all
  ENDIF

  sendcount = Nx * (myey_phys - mysy_phys + 1) * (myez_phys - mysz_phys + 1)
  CALL MPI_GATHERV(local,sendcount,PM_MPI_SINGLE_FLOAT_TYPE,                 &
  &                work,recvcounts,rdispl,PM_MPI_SINGLE_FLOAT_TYPE,          &
  &                sourcetask,comm,ierr )
...
END SUBROUTINE collect_phys_single
